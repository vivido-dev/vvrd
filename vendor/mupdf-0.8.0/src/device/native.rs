...
pub(crate) fn create<D: NativeDevice>(device: D) -> Result<Device, Error> {
    const {
        // MSVC's C headers do not define C11's max_align_t, so bindgen cannot
        // export it from mupdf-sys. MSVC's greatest fundamental C alignment is
        // the alignment of a double (8 bytes).
        #[cfg(target_env = "msvc")]
        assert!(align_of::<D>() <= align_of::<f64>());
